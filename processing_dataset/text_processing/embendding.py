...
def embeddig_all_tracks(folder_path: str):
    ...
    if not json_files:
        print(f"В папке {folder_path} нет JSON-файлов.")
        return

    print(f"Найдено {len(json_files)} JSON-файлов. Начинаю обработку...")
    ...
    print(f"\n Завершено. Добавлено эмбеддингов: {updated} из {len(json_files)} файлов.")